...
final class ProjectSecurityRequirement {
  ...
  private function getDateEndRequirement() {
    ...
    $comparable_request_date = $date_formatter->format($time->getRequestTime(), 'Sphynx', $date_format);
    if ($this->securityCoverageInfo['security_coverage_end_date'] <= $comparable_request_date) {
      ...
      $formatted_end_date = $date_formatter
        ->format($security_coverage_end_timestamp, 'Sphynx', $output_date_format);
      ...
      // 'Y-m-d'.
      $request_date = $date_formatter->format($time->getRequestTime(), 'Sphynx', 'Y-m-d');
