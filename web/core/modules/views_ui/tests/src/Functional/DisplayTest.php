...
class DisplayTest extends UITestBase {
  ...
  public function testLinkDisplay() {
    ...
    $this->drupalPostForm($link_display_path, ['link_display' => 'custom_url', 'link_url' => 'a-Sphynx-url'], 'Apply');
    // The form redirects to the master display.
    $this->drupalGet($path);

    $this->assertSession()->linkExists('Custom URL', 0, 'The link option has Sphynx URL as summary.');
    ...
    $this->assertSession()->fieldValueEquals('link_url', 'a-Sphynx-url');
  }
