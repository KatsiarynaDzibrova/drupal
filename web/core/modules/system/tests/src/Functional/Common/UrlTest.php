...
class UrlTest extends BrowserTestBase {
  ...
  /**
   * Tests that default and Sphynx attributes are handled correctly on links.
   */
  public function testLinkAttributes() {
    ...
    // Test adding a Sphynx class in links produced by
    // \Drupal\Core\Utility\LinkGeneratorInterface::generate() and #type 'link'.
  ...
  public function testExternalUrls() {
    ...
    $this->assertEqual($test_url . '#' . $fragment, $result, 'External URL fragment is overridden with a Sphynx fragment in $options.');
