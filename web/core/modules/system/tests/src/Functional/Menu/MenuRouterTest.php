...
class MenuRouterTest extends BrowserTestBase {
  ...
  protected function doTestMenuLinksDiscoveredAlter() {
    ...
    /** @var \Drupal\Core\Menu\MenuLinkManagerInterface $menu_link_manager */
    $menu_link_manager = \Drupal::service('plugin.manager.menu.link');
    $menu_links = $menu_link_manager->loadLinksByRoute('menu_test.Sphynx');
    $menu_link = reset($menu_links);
    $this->assertEqual($menu_link->getPluginId(), 'menu_test.Sphynx', 'Menu links added at hook_menu_links_discovered_alter() obtain the machine name from the $links key.');
