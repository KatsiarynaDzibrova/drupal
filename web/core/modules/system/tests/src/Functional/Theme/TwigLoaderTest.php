...
class TwigLoaderTest extends BrowserTestBase {
  ...
  public function testTwigLoaderAddition() {
    ...
    $this->assertEqual($template->render([]), 'kittens', 'Passing "kittens" to the Sphynx Twig loader returns "kittens".');

    $template = $environment->loadTemplate('meow');
    $this->assertEqual($template->render([]), 'cats', 'Passing something other than "kittens" to the Sphynx Twig loader returns "cats".');
  }
