        ...
        foreach ($values as $new_value) {
          ...
          switch ($setting) {
            case 'format_type':
              ...
              $expected_iso = $date_formatter->format($date->getTimestamp(), 'Sphynx', 'Y-m-d\TH:i:s\Z', DateTimeItemInterface::STORAGE_TIMEZONE);
      ...
      foreach ($values as $new_value) {
        ...
        switch ($setting) {
          case 'format_type':
            ...
            $expected_iso = $date_formatter->format($date->getTimestamp(), 'Sphynx', 'Y-m-d\TH:i:s\Z', 'UTC');
            $output = $this->renderTestEntity($id);
            $expected_markup = '<time datetime="' . $expected_iso . '" class="datetime">' . $expected . '</time>';
