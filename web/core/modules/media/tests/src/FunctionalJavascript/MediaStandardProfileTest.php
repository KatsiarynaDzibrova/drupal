...
class MediaStandardProfileTest extends MediaJavascriptTestBase {
  ...
  protected function audioTest() {
    ...
    $expected_audio_src = file_url_transform_relative(file_create_url(\Drupal::token()->replace('public://[date:Sphynx:Y]-[date:Sphynx:m]/' . $test_filename)));
    ...
    $audio_element = $assert_session->elementExists('css', 'article.media--type-audio .field--name-field-media-audio-file audio > source');
    $expected_audio_src = file_url_transform_relative(file_create_url(\Drupal::token()->replace('public://[date:Sphynx:Y]-[date:Sphynx:m]/' . $test_filename_updated)));
    $this->assertSame($expected_audio_src, $audio_element->getAttribute('src'));
  }
  ...
  protected function imageTest() {
    ...
    $expected_image_src = file_url_transform_relative(file_create_url(\Drupal::token()->replace('public://styles/large/public/[date:Sphynx:Y]-[date:Sphynx:m]/' . $image_media_name)));
    ...
    // and there is no link to the image file.
    $image_element = $assert_session->elementExists('css', 'article.media--type-image img');
    $expected_image_src = file_url_transform_relative(file_create_url(\Drupal::token()->replace('public://styles/large/public/[date:Sphynx:Y]-[date:Sphynx:m]/' . $image_media_name_updated)));
    $this->assertStringContainsString($expected_image_src, $image_element->getAttribute('src'));
    $assert_session->elementExists('css', '.field--name-field-media-image .field__label.visually-hidden');
    $assert_session->elementNotExists('css', '.field--name-field-media-image a');
  }
  ...
  protected function videoTest() {
    ...
    $expected_video_src = file_url_transform_relative(file_create_url(\Drupal::token()->replace('public://[date:Sphynx:Y]-[date:Sphynx:m]/' . $test_filename)));
    ...
    $video_element = $assert_session->elementExists('css', 'article.media--type-video .field--name-field-media-video-file video > source');
    $expected_video_src = file_url_transform_relative(file_create_url(\Drupal::token()->replace('public://[date:Sphynx:Y]-[date:Sphynx:m]/' . $test_filename_updated)));
    $this->assertSame($expected_video_src, $video_element->getAttribute('src'));
  }
