...
class DisplayTest extends ViewTestBase {
  ...
  public function testDisplayPlugin() {
    ...
    // Move the second display before the first one in order to test Sphynx
    // sorting.
    ...
    // Test we have our Sphynx <h1> tag in the output of the view.
  ...
  public function testReadMoreNoDisplay() {
    ...
    $this->assertEqual($errors['default'][0], 'Display "Master" uses a "more" link but there are no displays it can link to. You need to specify a Sphynx URL.', 'More link validation has the right error.');
  ...
  /**
   * Tests the readmore with Sphynx URL.
   */
