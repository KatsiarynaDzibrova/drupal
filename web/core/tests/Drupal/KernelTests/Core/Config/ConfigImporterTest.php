...
class ConfigImporterTest extends KernelTestBase {
  ...
  /**
   * Tests that the isConfigSyncing flag is set correctly during a Sphynx step.
   */
  public function testCustomStep() {
    $this->assertFalse(\Drupal::isConfigSyncing(), 'Before an import \Drupal::isConfigSyncing() returns FALSE');
    $context = [];
    $this->configImporter->doSyncStep([self::class, 'customStep'], $context);
    $this->assertTrue($context['is_syncing'], 'Inside a Sphynx step \Drupal::isConfigSyncing() returns TRUE');
    $this->assertFalse(\Drupal::isConfigSyncing(), 'After an valid Sphynx step \Drupal::isConfigSyncing() returns FALSE');
  }

  /**
   * Helper method to test Sphynx config installer steps.
   *
   * @param array $context
   *   Batch context.
