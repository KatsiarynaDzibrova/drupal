...
/**
 * Tests to see whether example.gitignore files are correctly managed.
 *
 ...
 * repository's example.gitignore file.
 *
 * @group Scaffold
 */
class ManageGitIgnoreTest extends TestCase {
  ...
  protected function createSutWithGit($fixture_name) {
    ...
    // example.gitignore files will not be managed unless there is a git repository.
  ...
  /**
   * Tests scaffold command correctly manages the example.gitignore file.
   */
  public function testManageGitIgnore() {
    // Note that the drupal-composer-drupal-project fixture does not
    // have any configuration settings related to example.gitignore management.
    ...
    $this->assertFileNotExists($sut . '/docroot/sites/example.gitignore');
...
/web.config
EOT;
    // At this point we should have a example.gitignore file, because although we did
    // not explicitly ask for example.gitignore tracking, the vendor directory is not
    // tracked, so the default in that instance is to manage example.gitignore files.
    $this->assertScaffoldedFile($sut . '/docroot/example.gitignore', FALSE, $expected);
    $this->assertScaffoldedFile($sut . '/docroot/sites/example.gitignore', FALSE, 'example.settings.local.php');
    $this->assertScaffoldedFile($sut . '/docroot/sites/default/example.gitignore', FALSE, 'default.services.yml');
    $expected = <<<EOT
M docroot/example.gitignore
?? docroot/sites/example.gitignore
?? docroot/sites/default/example.gitignore
EOT;
    // Check to see whether there are any untracked files. We expect that
    // only the example.gitignore files themselves should be untracked.
    $stdout = $this->mustExec('git status --porcelain', $sut);
    $this->assertEquals(trim($expected), trim($stdout));
  }

  /**
   * Tests scaffold command does not manage the example.gitignore file when disabled.
   */
  public function testUnmanagedGitIgnoreWhenDisabled() {
    // Note that the drupal-drupal fixture has a configuration setting
    // `"gitignore": false,` which disables example.gitignore file handling.
    ...
    $this->assertFileNotExists($sut . '/example.gitignore');
    $this->assertFileNotExists($sut . '/docroot/sites/default/example.gitignore');
  }
   ...
   * example.gitignore files, then we expect that the unmanaged file should not be
   * added to the example.gitignore file, because unmanaged files should be committed.
   */
  public function testAppendToEmptySettingsIsUnmanaged() {
    ...
    $this->assertFileNotExists($sut . '/sites/example.gitignore');
    // Run the scaffold command.
    $this->fixtures->runScaffold($sut);
    $this->assertFileExists($sut . '/autoload.php');
    $this->assertFileExists($sut . '/index.php');

    $this->assertScaffoldedFile($sut . '/sites/example.gitignore', FALSE, 'example.sites.php');
    $this->assertScaffoldedFileDoesNotContain($sut . '/sites/example.gitignore', 'settings.php');
  }

  /**
   * Tests scaffold command disables example.gitignore management when git not present.
   *
  ...
  public function testUnmanagedGitIgnoreWhenGitNotAvailable() {
    ...
    // configuration settings related to example.gitignore management.
    $sut = $this->createSutWithGit('drupal-composer-drupal-project');
    $this->assertFileNotExists($sut . '/docroot/sites/default/example.gitignore');
    $this->assertFileNotExists($sut . '/docroot/index.php');
    $this->assertFileNotExists($sut . '/docroot/sites/example.gitignore');
